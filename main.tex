...
\institute{Master's Thesis in Banking and Quantitative Finance \\
           Supervisor: Manuel \'Angel Dom\'inguez Toribio}
\date{May 2026}
...
\begin{frame}{Prior work: one variance formula per model}
  ...
  \begin{tabular}{@{}ll@{}}
    \toprule
    Model & $V_{\widehat{\mathrm{SR}}}$ \\
    \midrule
    IID Normal       & $1 + \tfrac{1}{2}\mathrm{SR}^2$ \\[4pt]
    IID Non-Normal   & $1 - \gamma\,\mathrm{SR} + \tfrac{\kappa-1}{4}\mathrm{SR}^2$ \\[4pt]
    AR(1)            & $\dfrac{1+\phi}{1-\phi} + \tfrac{1}{2}\dfrac{1+\phi^2}{1-\phi^2}\mathrm{SR}^2$ \\[8pt]
    GARCH(1,1)       & $1 - \dfrac{1-\beta}{1-\alpha-\beta}\gamma\mathrm{SR}
                        + \dfrac{\kappa-1}{4}
                          \dfrac{(1-\beta)^2(1+\alpha+\beta)}
                                {(1-\alpha-\beta)(1-2\alpha\beta-\beta^2)}\mathrm{SR}^2$ \\
    \bottomrule
  \end{tabular}
...
\begin{frame}{Contribution: closing the gap --- AR(1)-GARCH(1,1)}
  ...
  \begin{block}{Asymptotic variance, AR(1)-GARCH(1,1), general innovations}
    \[ V_{\widehat{\mathrm{SR}}} = \frac{1+\phi}{1-\phi}
       \;-\; b\,\gamma\,\mathrm{SR}
       \;+\; \tfrac{1}{4}\bigl(c_0 + c_\kappa\,\kappa + c_s\,\gamma^2\bigr)\,\mathrm{SR}^2 \]
  \end{block}
...
\begin{frame}{Which parameters move the variance?}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.3cm,keepaspectratio]{figures/oat_sensitivity.pdf}\\
      {\footnotesize One-at-a-time sweep}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.3cm,keepaspectratio]{figures/mc_sensitivity.pdf}\\
...
\begin{frame}{Two findings: sign asymmetry \& the AR-GARCH gap}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.2cm,keepaspectratio]{figures/coverage_ar.pdf}\\
      {\footnotesize AR(1) DGP: $\rho=+0.6$ vs $\rho=-0.6$}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.2cm,keepaspectratio]{figures/coverage_argarch.pdf}\\
...
\begin{frame}{Where the asymptotics strain}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.2cm,keepaspectratio]{figures/coverage_garch_persist.pdf}\\
      {\footnotesize GARCH persistence $\alpha+\beta$}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.2cm,keepaspectratio]{figures/coverage_tdf.pdf}\\
      {\footnotesize Student-$t$ degrees of freedom $\nu$}
    \end{column}
  \end{columns}
  \vspace{0.3em}
  \begin{itemize}
    \item Higher AR/GARCH persistence $\Rightarrow$ \textbf{slower} convergence to nominal.
    \item \alert{No fourth moment} ($\nu<4$): coverage \textbf{drifts away} as $T$ grows.
...
\begin{frame}{The corrections converge faster}
  \centering
  \includegraphics[width=\linewidth,height=5.5cm,keepaspectratio]{figures/bias_corrections.pdf}
...
\begin{frame}{Real data: three SPDR ETFs (2016--2025, weekly)}
  \centering
  \includegraphics[width=\linewidth,height=3.2cm,keepaspectratio]{figures/etf_prices_returns.pdf}
...
\begin{frame}{Efficiency vs robustness, in one picture}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.6cm,keepaspectratio]{figures/ellipse_gaussian.pdf}\\
      {\footnotesize Gaussian returns: parametric ellipse \textbf{inside}
      the non-parametric one. Coverage 95.0\% vs 95.4\%.}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=4.6cm,keepaspectratio]{figures/ellipse_studentt.pdf}\\
