...
\begin{frame}{Measuring reliability: the asymptotic variance}
  ...
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=\linewidth,height=3cm,keepaspectratio]{figures/price_series2.pdf}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
...
\begin{frame}{Prior work: one variance formula per model}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Model & $V_{\widehat{SR}}$ \\
    \midrule
    IID Normal       & $1 + \frac{1}{2}SR^2$ \\[4pt]
    IID Non-Normal   & $1 - \gamma\,SR + \tfrac{\kappa-1}{4}SR^2$ \\[4pt]
    AR(1)            & $\frac{1+\phi}{1-\phi} - \frac{1 + \phi +\phi^2}{1 - \phi^2}\gamma SR + \frac{1+\phi^2}{1-\phi^2} \frac{\kappa - 1}{4}SR^2$ \\[8pt]
    GARCH(1,1)       & $1 - \frac{1-\beta}{1-\alpha-\beta}\gamma SR + \frac{\kappa-1}{4}\frac{(1-\beta)^2 (1+\alpha+\beta)}{(1-\alpha-\beta)(1-2\alpha\beta-\beta^2)}SR^2$ \\
    \bottomrule
  \end{tabular}
...
\end{frame}%add the following footnotes \item[a] \parencite{Lo2002}\item[b] \parencite{Mertens2002}\item[c] \parencite{Lopez2025}\item[d] \parencite{Lopez2026}
...
\begin{frame}{Contribution: closing the gap --- AR(1)-GARCH(1,1)}
  ...
  \begin{block}{Asymptotic variance, AR(1)-GARCH(1,1), general innovations}
    \[ V_{\widehat{\mathrm{SR}}} = \frac{1+\phi}{1-\phi}
       \;-\; b\,\gamma\,\mathrm{SR}
       \;+\; \tfrac{1}{4}c\,\mathrm{SR}^2 \]
  \end{block}
  %b is $$
  %S_{12} \;=\; \frac{\sigma^3\,\operatorname{skew}_r\,(1+\phi+\phi^2)}
  %                  {(1-\phi^2)\,(1+2\phi\alpha-\phi\beta)}
  %\left[\,2\phi\alpha
  %  \;+\; \frac{(1-\beta)\bigl(1-\phi(\alpha+\beta)\bigr)}
  %            {1-\alpha-\beta}\,\right].\;
  %$$
  and c is $\frac{1}{1-\phi^2}(4\phi^2 + \kappa \frac{\Sigma}{1+X}+ \gamma^2 Z(N-\frac{M}{1+X}\Sigma))$ 
  
  aux vars:$M=1+\phi^2(2\alpha-\beta)$ $N=2\phi^2\alpha+\frac{1-\beta}{1-\alpha-\beta}(1-\phi^2 (\alpha+\beta))$
  $\Sigma=\frac{2}{3}X + \frac{(1-\beta)^2}{(1-\alpha-\beta)^2}\frac{1-(\alpha+\beta)^2}{1-2\alpha \beta- \beta^2}$ $Z=\frac{2}{3}\alpha\frac{(1-\phi(\alpha+\beta))(1+\phi+\phi^2)^2}{(1+\phi)^2(1+2\phi\alpha-\phi\beta)^2}\frac{4\phi}{1-\phi^2(\alpha+\beta)}$ %write the boxed formula again with the above auxs

  {\small Method of moments + delta method; all dependence packed into the
  long-run variance matrix $S$. \emph{Semi}-closed-form: odd moments of
  $\sigma_t^2$ via Taylor expansion.}

  \vspace{0.3em}
  {\small Lo, Mertens, AR(1), GARCH(1,1) all recovered as restrictions
  ($\phi=0$ or $\alpha=\beta=0$).}
\end{frame}
...
\begin{frame}{The corrections converge faster}
  \centering
  %prev version: \includegraphics[width=\linewidth,height=5.5cm,keepaspectratio]{figures/bias_corrections.pdf}
  %use this figures instead:
  % \begin{figure}[H]
  %   \centering
  %   \begin{subfigure}{0.3\textwidth}
  %     \centering
  %     \includegraphics[width=\linewidth]{images/bias/iidnormal.png}
  %     \caption{IID Normal}
  %   \end{subfigure}
  %   \begin{subfigure}{0.3\textwidth}
  %     \centering
  %     \includegraphics[width=\linewidth]{images/bias/iidnonnormal.png}
  %     \caption{IID Non-Normal}
  %   \end{subfigure}
  %   \begin{subfigure}{0.3\textwidth}
  %     \centering
  %     \includegraphics[width=\linewidth]{images/bias/arnormal.png}
  %     \caption{AR(1) Normal}
  %   \end{subfigure}
  %   \vspace{0.5cm}
  %   \begin{subfigure}{0.3\textwidth}
  %     \centering
  %     \includegraphics[width=\linewidth]{images/bias/arnonnormal.png}
  %     \caption{AR(1) Non-Normal}
  %   \end{subfigure}
  %   \begin{subfigure}{0.3\textwidth}
  %     \centering
  %     \includegraphics[width=\linewidth]{images/bias/garch.png}
  %     \caption{GARCH(1,1)}
  %   \end{subfigure}
  %   \begin{subfigure}{0.3\textwidth}
  %     \centering
  %     \includegraphics[width=\linewidth]{images/bias/argarch.png}
  ...
  \vspace{0.4em}
  {\small Corrected estimator (blue) converges faster in every model.
...
\begin{frame}{Real data: three SPDR ETFs (2016--2025, weekly)}
  \centering
  \includegraphics[width=\linewidth,height=3.2cm,keepaspectratio]{figures/prices.png}%add also figures/returns.png to the side
...
\begin{frame}{Beyond one metric: joint distribution with Value-at-Risk}
  {\small Pair SR with VaR; derive the joint asymptotic distribution
  (MM + multivariate delta method).}
