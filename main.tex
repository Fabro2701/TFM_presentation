...
\begin{frame}{Contribution: closing the gap --- AR(1)-GARCH(1,1)}
  ...
  {\small Here $b$ and $c$ pack every serial-dependence and heteroskedasticity
  effect into the interval width; their full expressions are on the next slide.}
...
% ---------- SLIDE 5b — Coefficients in full ----------
\begin{frame}{AR(1)-GARCH(1,1): the coefficients in full}
  \footnotesize
  \setlength{\parskip}{0pt}%
  \setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}%
  \setlength{\abovedisplayshortskip}{1pt}\setlength{\belowdisplayshortskip}{1pt}%

  \textbf{Skewness channel} (the $b\,\gamma\,\mathrm{SR}$ term), through $S_{12}$:
  \[
    S_{12}=\frac{\sigma^{3}\,\gamma\,(1+\phi+\phi^{2})}
                {(1-\phi^{2})(1+2\phi\alpha-\phi\beta)}
           \left[\,2\phi\alpha
             +\frac{(1-\beta)\bigl(1-\phi(\alpha+\beta)\bigr)}{1-\alpha-\beta}\,\right]
  \]

  \textbf{Kurtosis coefficient:}\quad
  $\displaystyle c=\frac{1}{1-\phi^{2}}\!\left(4\phi^{2}
      +\kappa\,\frac{\Sigma}{1+X}
      +\gamma^{2}Z\Bigl(N-\frac{M}{1+X}\,\Sigma\Bigr)\right)$

  \vspace{0.1em}
  \textbf{Auxiliary terms:}
  \[
    M = 1+\phi^{2}(2\alpha-\beta),\qquad
    N = 2\phi^{2}\alpha+\frac{1-\beta}{1-\alpha-\beta}\bigl(1-\phi^{2}(\alpha+\beta)\bigr)
  \]
  \[
  ...
  \vspace{0.2em}
  \normalsize
  \begin{block}{Explicit asymptotic variance (auxiliary terms substituted)}
    \[
      \resizebox{\textwidth}{!}{$\displaystyle
      V_{\widehat{\mathrm{SR}}}=\frac{1+\phi}{1-\phi}
        -b\,\gamma\,\mathrm{SR}
        +\frac{\mathrm{SR}^{2}}{4(1-\phi^{2})}
         \left(4\phi^{2}+\kappa\frac{\Sigma}{1+X}
               +\gamma^{2}Z\Bigl(N-\frac{M}{1+X}\Sigma\Bigr)\right)$}
    \]
  \end{block}

  \vspace{-0.4em}
  {\scriptsize $\gamma=$ skewness,\; $\kappa=$ kurtosis,\; $\sigma,\,X$ as defined in the text.}
\end{frame}
