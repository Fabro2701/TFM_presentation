      ...
      V_{\widehat{\mathrm{SR}}}=\frac{1+\phi}{1-\phi}
        ...
        +\frac{1}{4(1-\phi^{2})}
         \left[4\phi^{2}+((1+\phi^2)\kappa -1-5\phi^2)\frac{\Sigma}{1+X}
               +\gamma^{2}Z\Bigl(N-\frac{\Sigma}{1+X}U\Bigr)\right]\mathrm{SR}^{2}$}
    \]
  \end{block}
...
\begin{frame}{Which parameters move the variance?}
  ...
  \begin{itemize}
    ...
    \item Kurtosis, skewness (steady, $\sim$linear), and $\phi$ (asymmetric, quadratic) next.
    \item $\alpha,\beta$: mild on average, \textbf{explosive near the boundaries}
...
\begin{frame}{Testing the intervals: coverage on synthetic data}
  Simulate Monte Carlo trajectories from a known process; measure how often the 95\% interval contains
  the true SR.
...
\begin{frame}{Two findings: sign asymmetry \& the AR-GARCH gap}
  ...
  \pause
  \begin{itemize}
    \item Autocorrelation is \alert{asymmetric in sign}: negative $\rho \Rightarrow$
          uncoverage; positive $\rho \Rightarrow \alert{correct}$.
...
\begin{frame}{Where the asymptotics fail}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}\centering
...
\begin{frame}{The point estimate is biased and a popular fix is wrong}
  ...
  Christie (2005): $\mathrm{SR}\bigl(1+\tfrac{1}{2T}\bigr)$ \alert{too small}.
  Correct: $\mathrm{SR}\bigl(1+\tfrac{3}{4T}\bigr)$. 
  
  General correction via the same $S$:
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    AR(1)-GARCH(1,1) & $\!\begin{array}[c]{@{}l@{}} \displaystyle \left(\widehat{SR}+\frac{\gamma}{2T}\,\frac{1+\phi+\phi^{2}}{(1-\phi^{2})(1+2\phi\alpha-\phi\beta)}\left[2\phi\alpha + \frac{(1-\beta)\bigl(1-\phi(\alpha+\beta)\bigr)}{1-\alpha-\beta}\right]\right) \\[2.5ex] \displaystyle \quad \times \left[1+\frac{\phi}{(T-1)(1-\phi)}+\frac{3}{8T(1-\phi^{2})}\left(4\phi^{2} + \frac{\Sigma}{1+X}\,K + \gamma^{2}Z\left(N - \frac{\Sigma}{1+X}\,U\right)\right)\right]^{-1} \end{array}$ \\
    \bottomrule
  \end{tabular}}
  \end{center}
...
\begin{frame}{Real data: three SPDR ETFs (2016--2025, weekly)}
  ...
  \begin{tabular}{@{}lcccl@{}}
    \toprule
    Series & Ljung--Box & ARCH-LM & Normality (JB) & Selected specification \\
    \midrule
...
\begin{frame}{Beyond one metric: joint distribution with Value-at-Risk}
  ...
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      \begin{itemize}\small
        \item \alert{Negative covariance} ($\tfrac{\mu z_\alpha}{2}-\sigma<0$):
              overestimation of $\widehat{SR}$ comes together with an underestimation of $\widehat{VaR}$.
    ...
    \end{column}
  \end{columns}
  {\scriptsize \quad $\varphi(\cdot)$ = standard normal pdf,\; $\Phi(\cdot)$ = standard normal cdf,\; $z_\alpha=\Phi^{-1}(\alpha)$.}
\end{frame}
...
\begin{frame}{Efficiency vs robustness, in one picture}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}\centering
      ...
      {\footnotesize Student-$t_6$ returns: parametric ellipse \alert{biased}, coverage \alert{14.8\%}; non-parametric stays \alert{95.4\%}.}
    \end{column}
  \end{columns}
  \vspace{0.4em}
  \begin{center}
  Parametric wins when the assumption is right; fails badly when it is wrong.
  \end{center}
\end{frame}
...
\begin{frame}{How reliable is this number?}
  \begin{enumerate}
    ...
    \item In \textbf{small samples} the estimate is biased.
